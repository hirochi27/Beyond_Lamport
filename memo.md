...
## 現状
シーケンサは、プローブのsampleを保存しているだけ。
offset分布は```distribution_for(client_id)```を呼んで、RTT中央値の計算、RTT外れ値を除外、offset平均を計算
標準偏差を計算、uncertaintyを計算、分布を作る、といったことをしている

作られた分布は、クライアント側のdistribution変数に一時的に保存
distribution = 中央値と広がり（正規分布にしか対応してない）


各クライアント同士の分布の差
  １事前に、全ペア同士の分布の計算をしておく
        イベント発生時の確率計算が高速
        ペア数が O(n2) に増える→メモリ、前処理時間が膨大
        実装は楽
        今後クライアント数を増やすならやめた方がいい
  ２連絡が来る度に計算
        比べることになったときに分布を計算
        同じペアを何度も比較する場合、毎回計算が発生（キャッシュで改善可能/何分以上経過してたら計算し直す、みたいな作業を入れたい）

クライアントの分布が正規分布前提なら、標準偏差と平均を保存するだけでOK
  →正規分布以外の場合、制度が落ちる
  →これを、連絡がきた最初に計算。その後キャッシュ/経過時間ごとに改善。が良いと思ったけど、計測してみないと分かんない
...
プロトタイプは、正規分布前提
  ...
  10ノードくらいが限界かも？
    ...
    イベント処理速度
    クライアント数に対する増加率
    を測れば、後からオンデマンド方式や階層型と比較しやすくなります。

今はクライアントA→B、B→A両方の差分布を計算している
  →とりあえずこのままやる。
  →クライアント数増やすなら、片方だけにして、半分の計算量にする

プローブを全部やってから、分布の差を計算。
プローブの変化に対応できない

グラフを作成する段階での、比較対象をどう決めるか


1. 全ペアについて P(A before B) を計算する
2. 各ペアで確率が高い向きを仮採用する
3. 有向グラフを作る
4. DAG ならそのままトポロジカルソート
5. サイクルがあるなら、信頼度の低いエッジから外す
6. DAG になったらトポロジカルソート


今作ってるオフセット分布のヒストグラムから、オフセット分布の平均がほぼ0になることを確認したい
同じPC内で動かしてるから、オフセット分布は0でOKなはず


メッセージが来た場合、Δθを使って、全メッセ―ジペアの先行確率を計算
→フルメッシュの完全グラフ
→確率の高い方で矢印を作る
→フルメッシュ完全有向グラフ
トポロジカルソートしたい
